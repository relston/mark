...
### [0.11.0] - 2025-12-26
#### Changed
- Replaced web scraping implementation: migrated from `pyppeteer` + `markdownify` + `beautifulsoup4` to `crawl4ai`
- Improved markdown extraction quality using crawl4ai's built-in markdown conversion
- Enhanced error handling with clearer messages when browsers are missing

#### Removed
- Removed `pyppeteer` dependency
- Removed `markdownify` dependency  
- Removed `beautifulsoup4` dependency
- Removed `soup` attribute from `Page` class (no longer needed)

#### Added
- Added `crawl4ai` dependency for web scraping
- Added `mark-setup-browsers` CLI command to install Playwright browsers required for web scraping
- Added automatic browser detection and helpful error messages when browsers are missing
...
### [0.10.0] - 2025-02-18
#### Added
- `mark models` subcommand will list all available LLM models available
...
#### Removed
- Import direct import of `openai` (referencing LLMs dependencies directly)

### [0.8.3] - 2025-01-20
#### Removed
- Import of `image_to_data_url` has been removed from LangChain.
